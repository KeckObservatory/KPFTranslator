    ...
    def perform(cls, args, logger, cfg):
        ...
        for i,xi in enumerate(xis):
            for j,yi in enumerate(yis):
                ...
                cond = '($kpffiu.ADC1STA == Ready) and ($kpffiu.ADC2STA == Ready)'
                success = ktl.waitFor(cond, timeout=2*max([dx*nx, dy*ny])/5)
...
#                         kpffvc[f"{camera}EXPOSE"].write('yes', wait=False)
    ...
    def add_cmdline_args(cls, parser, cfg=None):
        ...
        args_to_add['nx'] = {'type': int,
                    'help': 'Number of grid points for ADC prism 1'}
        args_to_add['ny'] = {'type': int,
                    'help': 'Number of grid points for ADC prism 2'}
        args_to_add['dx'] = {'type': float,
                    'help': 'Distance (arcsec) between grid points for ADC prism 1'}
        args_to_add['dy'] = {'type': float,
                    'help': 'Distance (arcsec) between grid points for ADC prism 2'}
